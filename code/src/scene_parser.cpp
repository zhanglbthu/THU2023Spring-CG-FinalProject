...
void SceneParser::parseFile()
{
    ...
    while (getToken(token))
    {
        ...
        else if(!strcmp(token, "DepthCamera"))
        {
            parseDepthCamera();
        }
...
void SceneParser::parseDepthCamera()
{
    char token[MAX_PARSER_TOKEN_LENGTH];
    // read in the camera parameters
    getToken(token);
    ...
    getToken(token);
    assert(!strcmp(token, "}"));
    camera = new DepthCamera(center, direction, up, width, height, angle_radians, focalLength, aperture);
}
...
Light *SceneParser::parseDiskLight()
{
    ...
    getToken(token);
    assert(!strcmp(token, "}"));
    return new DiskLight(position, normal, color, radius, emission);
}
// ====================================================================
// ================}====================================================
...
Material *SceneParser::parseMaterial()
{
    ...
    while (true)
    {
        ...
        else if (strcmp(token, "reflectIntensity") == 0)
        {
            reflectIntensity = readFloat();
        }
        else if (strcmp(token, "refractIntensity") == 0)
        {
            refractIntensity = readFloat();
        }
    ...
    // Material *answer = new Material(diffuseColor, specularColor, shininess, refractiveIndex, reflectIntensity, refractIntensity, filename);
    Material *answer = new Material(color, emission, type, refractiveIndex, filename);
